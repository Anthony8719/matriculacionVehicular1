...
float calcularAvaluo(vehiculo v){
	...
	// Función para obtener la fecha actual para elcomprobante
	...
	void calcularBeneficiosPorHistorial(vehiculo *v) {
		...
		if(v->descuentoPorHistorial > 10.0f) {
			v->descuentoPorHistorial = 10.0f;//Evita que por el descuento el valor de la matrícula sea negativo
		}
	}
	...
	int evaluarRevisionTecnica(vehiculo *v) {
		...
		const char *categorias[8] = {
			"Frenos",
				...
				"Cinturones/Casco",//Forma general para cualquier tipo de vehículo
		...
		int puntajeTotal = 0;
		for (int i = 0; i < 8; i++) {
			// Cambiamos la pregunta si es moto (tipo 0) para que digacasco y no cinturón
			if (i == 5 && v->tipo == 0) {  
				limpiarPantalla();
